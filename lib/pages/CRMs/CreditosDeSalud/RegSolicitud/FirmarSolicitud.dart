...
import 'FirmarSolicitud.dart';

class FirmarSolicitud extends StatefulWidget {
  const FirmarSolicitud({super.key});

  @override
  State<FirmarSolicitud> createState() => _FirmarSolicitudState();
}
...
class _FirmarSolicitudState extends State<FirmarSolicitud> {
  ...
  Widget build(BuildContext context) {
    return MyCustomFormFirmarSolicitud();
    // return Scaffold(
    //   appBar: AppBar(
...
class MyCustomFormFirmarSolicitud extends StatefulWidget {
  const MyCustomFormFirmarSolicitud({super.key});

  @override
  MyCustomFormFirmarSolicitudState createState() {
    return MyCustomFormFirmarSolicitudState();
  }
}

class MyCustomFormFirmarSolicitudState
    extends State<MyCustomFormFirmarSolicitud> {
  ...
  void Consultar(IDLR,IDInfo) async { 
    ...
    if(IDLR!=0 && IDInfo!=0){    
      try {
        var url = Uri.https('fasoluciones.mx', 'ApiApp/Solicitud/FirmarFAD.php');
        var data ={
          'Pantalla':'Firmar',
          'id_LR': "$IDLR",
          'id_info': "$IDInfo"
  ...
  void Ingresar(Pantalla, IDLR, IDInfo) async {
    try {
var url = Uri.https('fasoluciones.mx', 'ApiApp/Solicitud/Actualizar.php');
 ...
 //se usa para mostrar los datos del estado
  ...
  Future<void> mostrar_datos() async {
    ...
    var id_LR = await prefs.getInt('id_LR');
      ...
      child: ListTile(
        ...
        onTap: () {
          ...
          Navigator.push(context,
              MaterialPageRoute(builder: (context) => PerfilSolVerificar()));
        },
      ),
    );
  }
